...
function BillingPage() {
    ...
    const handleChange = async (e) => {
        ...
        if (name === 'product_name') {
            try {
                const response = await axios.get(`http://localhost:4041/api/customers/getProductDetails?productName=${value}`);
    ...
    <thead>
        <tr>
            <th>Product ID</th>
            <th>Product Name</th> {/* Add this new column */}
            <th>Quantity</th>
            <th>Price</th>
            <th>Total</th>
        </tr>
    ...
    <tbody>
        {billingItems.map((item, index) => (
            <tr key={index}>
                ...
                <td>{item.selectedProductName}</td> {/* Render selectedProductName */}
                <td>{item.quantity}</td>
                <td>{item.price}</td>
                <td>{item.total}</td>
            </tr>
